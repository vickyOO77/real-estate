...
export default function SearchFilters() {
  ...
  const searchProperties = (filterValues) => {
    ...
    values.forEach((item) => {
      if(item.value && filterValues?.[item.name]) {
        query[item.name] = item.value  //inserting queries 
      }
    })    
    router.push({ pathname: path, query: query }); //changing URL by passing path and query
  };
  ...
  return (
    <Flex bg='gray.100' p='4' justifyContent='center' flexWrap='wrap' position='sticky' top='0.5' zIndex={1} >
      {filters?.map((filter) => (
